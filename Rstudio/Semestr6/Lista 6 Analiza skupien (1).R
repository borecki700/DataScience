...
# Zad. 1 Przeprowadź analizę skupień niehierarchiczną metodą k-średnich na
# zbiorze danych uber załączonym w materiałach. Zinterpretuj wyniki. Przed
# staw je za pomocą ciekawej formy graficznej (ciekawych form graficznych).
# Zwróć uwagę na rozmiar danych. Jakie rozwiązanie proponujesz, aby przy
# mniej wydajnym sprzęcie komputerowym uzyskać dostatecznie dobre wykre
# sy?
...
###
file.exists("uber-raw-data-apr14.csv")
list.files()
getwd()
setwd("C:/Users/Kamil/OneDrive/Pulpit/Semestr 6/Borecki R Studio")
file.exists("uber-raw-data-apr14.csv")
###
...
# Zad. 2 Przeprowadź analizę skupień na zbiorze danych ruspini z pakie
# tu cluster dwiema różnymi metodami hierarchicznymi- z użyciem me
# tody pojedynczego wiązania i pełnego wiązania. Przedstaw opracowywane
# dane na wykresie. Zinterpretuj otrzymane wyniki np. za pomocą funkcji
# cluster.stats. Przedstaw wyniki na dendrogramie i zaznacz na nim pro
# ponowany wybór klastrów. Porównaj graficznie wyniki uzyskane poszczegól
# nymi metodami (np. umieść na wykresie wszystkie obserwacje, oznaczając
# klastry przydzielone metodą pojedynczego wiązania odpowiednim kolorem,
# a metodą pełnego wiązania- kształtem).
...
# Zad. 3 Przeprowadź analizę skupień na zbiorze danych xclara z pakie
# tu cluster wybraną metodą hierarchiczną z zastosowaniem różnych miar
# odległości- Euklidesową i Manhattan. Przedstaw opracowywane dane na
# wykresie. Porównaj wyniki uzyskane przy użyciu różnych miar. Zinterpretuj
# otrzymane wyniki np. za pomocą funkcji cluster.stats. Przedstaw wyniki
# na dendrogramach z zaznaczeniem proponowanego wyboru klastrów oraz na
# wykresach skupień (wsk. np. funkcje clusplot i plotcluster). Zapoznaj się
# z podziałem na klastry zaproponowanym w dokumentacji funkcji
...
# Zad. 8 Wygeneruj 30 punktów na płaszczyźnie. Niech 10 z nich będzie
# skupionych wokół punktu (1,1), 10 wokół punktu (1,7) oraz 10 wokół (7,1).
# Można wybrać np. rozkład normalny z odpowiednią średnią i małą warian
# cją. Utwórz z tych punktów ramkę danych o 30 obserwacjach i kolumnach
# x i y. Dla tak przygotowanego zbioru opracuj wykres rozrzutu. Następnie
# przeprowadź analizę skupień metodą hierarchiczną i narysuj dendrogram.
...
dane<-data.frame(x=c(x1,x2,x3),y=c(y1,y2,y3))
plot(dane)

#dane<-scale(dane) usuwamy

#wyznaczamy macierz odległości
mac_odl<-dist(scale(dane),method="euclidean")

dane_hclust<-hclust(mac_odl,method="ward.D") #metoda Warda

plot(dane_hclust)
dane$groups<-factor(cutree(dane_hclust,k=3))

plot(dane$x,dane$y,col=dane$groups)


library("cluster")
clusplot(dane[,1:2],dane$groups)#,labels=2,lines=0,col=TRUE,shade=TRUE)
clusplot(dane,cutree(dane_hclust,k=3))

library(fpc)
plotcluster(dane[,1:2],dane$groups)




# Zad. 10 Przeprowadź hierarchiczną analizę skupień dla zbioru USArrest z
# metryką euklidesową i z zastosowaniem metody Warda. Wyniki przedstaw
# na dendrogramie oraz za pomocą funkcji fviz
# cluster
...
#date_dane<-(USArrests)
#fviz_cluster(list(data_dane[,1:2],cluster=dane$groups),repel=TRUE)

# ZADANIE 10
library(factoextra)   # do fviz_cluster i fviz_dend
library(datasets)     # zbiór USArrests

# 1. Wczytanie danych
dane <- USArrests

# 2. Standaryzacja danych (bardzo ważna!)
dane_scaled <- scale(dane)

# 3. Obliczenie macierzy odległości euklidesowej
dist_euclid <- dist(dane_scaled, method = "euclidean")

# 4. Hierarchiczne grupowanie - metoda Warda
hclust_ward <- hclust(dist_euclid, method = "ward.D2")

# 5. Dendrogram
fviz_dend(hclust_ward, k = 4, rect = TRUE, show_labels = TRUE)

# 6. Podział na klastry (np. 4 grupy)
grupy <- cutree(hclust_ward, k = 4)

# 7. Wizualizacja klastrów
fviz_cluster(list(data = dane_scaled, cluster = grupy))
